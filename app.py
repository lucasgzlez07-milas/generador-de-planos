    ...
    def inject_styles():
        st.markdown("""
            <style>
                ...
                .canvas-container {
                    background-color: #ffffff; background-image: radial-gradient(#d1d5db 1px, transparent 1px);
                    background-size: 20px 20px; border: 1px solid #e5e7eb; border-radius: 20px;
...
def reset_state():
    """Resetea el estado utilizando la clave del formulario para evitar StreamlitAPIException"""
    # Para resetear widgets en Streamlit 2026, lo más seguro es usar keys dinámicas o limpiar via session_state.
    # Pero la excepción ocurre al asignar directamente. La solución es vaciar y disparar rerun.
...
def main():
    ...
    with st.sidebar:
        ...
        # Usamos valores por defecto del session_state para que no explote al resetear
